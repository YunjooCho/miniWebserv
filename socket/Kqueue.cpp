...
int Kqueue::getNewEvents(void)
{
	return (m_newEvents);
}

struct kevent*	Kqueue::getCurrEvent(int idx)
{
...
void Kqueue::change_events( uintptr_t ident, int16_t filter, \
...
{
	m_newEvents = ::kevent(m_kqFd, &m_changeList[0], m_changeList.size(), m_eventList, 8, timeout);
	if (m_newEvents == -1)
...
void Kqueue::changeListClear(void)
{
	m_changeList.clear();
}
