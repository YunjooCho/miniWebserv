...
void ServerSocket::setClientDate(const int& clientFd, const std::string& data) {
  m_clients[clientFd] += data;
  
  //debugging
  std::cout << "received data from " << clientFd << ": " << m_clients[clientFd] << std::endl;
}

bool ServerSocket::findClient(const int& clientFd) {
  if (m_clients.find(clientFd) != m_clients.end()) return (true);
  return (false);
}
...
void ServerSocket::handleWriteEvent(const int& clientFd) {
  ...
  if (iter != m_clients.end() && m_clients[clientFd] != "") {
    ...
    if (write(clientFd, m_clients[clientFd].c_str(),
              m_clients[clientFd].size()) == -1) {
